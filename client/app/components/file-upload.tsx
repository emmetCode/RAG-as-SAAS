...
const FileUploadComponent: React.FC<DocumentUploadProps> = ({ onUploadSuccess }) => {
  ...
  const simulateProcessingSteps = () => {
    setTimeout(() => {
      setUploadStatus({ state: 'processing', message: 'Processing document...' });
    }, 1000);

    setTimeout(() => {
      setUploadStatus({ state: 'embedding', message: 'Creating vector embeddings...' });
    }, 1400);

    setTimeout(() => {
      setUploadStatus({ state: 'storing', message: 'Storing in database...' });
    }, 1400);
  ...
  const handleFiles = async (files: FileList) => {
    ...
    setTimeout(async () => {
      ...
      if (file.type !== 'application/pdf') {
        setUploadStatus({ state: 'error', message: 'Please upload a PDF file' });
        return;
      }

      if (file.size > 10 * 1024 * 1024) {
        setUploadStatus({ state: 'error', message: 'File size exceeds 15MB limit' });
        return;
      }

      try {
        setUploadStatus({ state: 'uploading', message: 'Uploading document...', progress: 0 });
        ...
        setUploadStatus({ state: 'error', message: 'Failed to upload document' });
      }
    }, 50);
  };
    ...
    <div className="space-y-6">
      ...
      <div className="mt-6">
        ...
        <div className="flex items-center justify-between relative">
          ...
          {(['upload', 'process', 'embedding', 'storage'] as const).map((step, index) => {
            const stepIcons = {
