    ...
    <div className="flex flex-col h-screen p-6 bg-slate-780">
      <div className="flex-1 overflow-y-auto pr-4 space-y-6 custom-scroll">
        {messages.map((msg, index) => {
          const isAssistant = msg.role === 'assistant';
          const isUser = msg.role === 'user';
          const showExpand = isAssistant && msg.content && shouldShowExpand(msg.content);
          ...
          return (
            <div key={index} className={`flex flex-col ${isUser ? 'items-end' : 'items-start'}`}>
              <div
                className={`relative max-w-xl p-4 rounded-2xl shadow-md text-sm ${messageBackgroundColor} ${messageTextColor} ${isUser ? 'ml-auto rounded-br-none' : 'mr-auto rounded-bl-none'
                  }`}
              >
                <div
